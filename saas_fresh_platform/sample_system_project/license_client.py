...
DEFAULT_STATE_DIR = Path.home() / 'Library' / 'Application Support' / 'SimpleInventoryDemo'
DEFAULT_STATE_DIR.mkdir(parents=True, exist_ok=True)
LOCAL_LICENSE_FILE = os.getenv('LOCAL_LICENSE_FILE', str(DEFAULT_STATE_DIR / 'license_state.json'))
...
def load_local_state():
    ...
    with open(LOCAL_LICENSE_FILE, 'r', encoding='utf-8') as file:
        return json.load(file)


def save_local_state(data):
    with open(LOCAL_LICENSE_FILE, 'w', encoding='utf-8') as file:
